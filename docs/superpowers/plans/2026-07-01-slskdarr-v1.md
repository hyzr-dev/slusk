...
- Produces: `slskd.Client`; `slskd.New(baseURL, apiKey string) *Client`; types `slskd.Result`, `slskd.Transfer`; methods `Search(ctx, query) ([]Result, error)`, `Enqueue(ctx, username, filename string) (string, error)` returning slskd's transfer id, `ListDownloads(ctx) ([]Transfer, error)`, `Cancel(ctx, username, id string) error`.

**Note on the slskd API assumption:** slskd assigns a transfer `id` (GUID) and `GET /api/v0/transfers/downloads` returns transfers grouped by user. If the real API shape differs, only this file changes — the `engine` port stays the same. Verify against the running slskd during implementation and adjust the JSON structs here.
...
// Search issues a search and returns the flattened result files.
func (c *Client) Search(ctx context.Context, query string) ([]Result, error) {
	var results []Result
	err := c.do(ctx, http.MethodPost, "/api/v0/searches", map[string]string{"searchText": query}, &results)
	return results, err
}

// Enqueue starts a download of one file from a user and returns slskd's id.
