...
func TestSchemaMigratesLegacyShape(t *testing.T) {
	...
	for _, stmt := range []string{
		// Scaffolding for candidate_attempts' FK below, not a faithful
		// pre-rewrite schema — but it must carry the columns 0001 assumes
		// already exist, or a later migration that indexes one of them fails
		// here for a reason no production database would ever hit. 0001 has
		// ADD COLUMN fixups only for what the rewrite *added* (retries,
		// not_before, failed_at, min/max_track_count); everything else,
		// including the artist columns, predates it.
